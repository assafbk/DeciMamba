...
def create_block(
    ...
    mixer_cls = partial(Mamba, layer_idx=layer_idx, model_name=model_name, decimation_config=decimation_config, **ssm_cfg, **factory_kwargs)
    norm_cls = partial(
...
def update_params_for_debug(params_for_debug, cur_params_for_debug, decimation_config):
...
class MixerModel(nn.Module):
    ...
    def forward(self, input_ids, inference_params=None, resp_len=None):        
        hidden_states = self.embedding(input_ids)
        residual = None
        params_for_debug = init_params_for_debug(False)
        for idx, layer in enumerate(self.layers):
            hidden_states, residual, cur_params_for_debug = layer(
                hidden_states, residual, inference_params=inference_params, resp_len=resp_len
            )
            if cur_params_for_debug is not None: # in recurrent mode we do not decimate
                residual = residual[:,cur_params_for_debug['not_decimated'],:]
            params_for_debug = update_params_for_debug(params_for_debug, cur_params_for_debug, self.decimation_config)
...
class MambaLMHeadModel(nn.Module, GenerationMixin):
    ...
    def forward(self, input_ids, position_ids=None, inference_params=None, num_last_tokens=0):
        ...
        """
        hidden_states, params_for_debug = self.backbone(input_ids, inference_params=inference_params)
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name, device=None, dtype=None, cache_dir=None, decimation_config=None, **kwargs):
        config_data = load_config_hf(pretrained_model_name)
        config = MambaConfig(**config_data)
        model = cls(config, device=device, dtype=dtype, decimation_config=decimation_config, **kwargs)
...
class DecimatingMambaModel(nn.Module, GenerationMixin):
    ...
    def forward(self, input_ids, position_ids=None, inference_params=None, num_last_tokens=0):
        ...
        num_last_tokens: if > 0, only return the logits for the last n tokens
        """
        hidden_states, params_for_debug = self.backbone(input_ids, inference_params=inference_params, resp_len=num_last_tokens)
        if num_last_tokens > 0:
            hidden_states = hidden_states[:, -num_last_tokens:]
        lm_logits = self.lm_head(hidden_states)
        CausalLMOutput = namedtuple("CausalLMOutput", ["logits"])
